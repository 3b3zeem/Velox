...
export const Header: React.FC = () => {
  ...
  return (
    ...
    >
      {/* LEFT SECTION: Brand, Projects & Auth */}
      <div className="flex items-center space-x-1.5 sm:space-x-2 shrink-0">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2">
          ...
          <div className="hidden sm:flex flex-col leading-tight">
            <span
              className={`font-black text-xs sm:text-sm tracking-tight leading-none ${isLight ? "text-slate-900" : "text-white"}`}
            >
              Velox
            </span>
          </div>
        ...
        {/* Projects & Auth Buttons — Always Visible & Mobile Responsive */}
        <div
          className={`flex items-center ml-0.5 sm:ml-1 pl-1.5 sm:pl-2 border-l space-x-2 sm:space-x-1.5 ${isLight ? "border-slate-200" : "border-slate-800"}`}
        >
          <button
            onClick={() => setProjectsModalOpen(true)}
            className={`flex items-center space-x-1 px-2 sm:px-2.5 py-1 rounded-xl text-xs font-semibold border transition-all cursor-pointer ${
              isLight
                ? "bg-slate-100 hover:bg-slate-200 text-slate-800 border-slate-300"
                : "bg-slate-950 hover:bg-slate-900 text-slate-200 border-slate-800"
            }`}
            title="Save / Open Projects & Sync Cloud"
          >
            <FolderOpen className="w-3.5 h-3.5 text-indigo-500 shrink-0" />
            <span className="hidden sm:inline">Projects</span>
          </button>

          {/* Dedicated Log In / Account Button — Icon only on mobile, text on sm+ */}
          <button
            onClick={() => setAuthModalOpen(true)}
            aria-label={user ? `Account (${user.email})` : "Log In / Sign Up"}
            className={`flex items-center space-x-1 p-1.5 sm:px-2.5 sm:py-1 rounded-xl text-xs font-semibold border transition-all cursor-pointer ${
              user
                ? isLight
            ...
            title={user ? `Logged in as ${user.email}` : "Log In / Sign Up"}
          >
            {user ? (
              <>
                {user.user_metadata?.picture ? (
                  <img
                    ...
                    className="w-4 h-4 rounded-full object-cover shrink-0 border border-emerald-400/60"
                  />
                ) : (
                  <>  
                    <UserCheck className="w-3.5 h-3.5 text-emerald-500 shrink-0" />
                    <span className="hidden sm:inline truncate max-w-[85px] font-bold">
        ...
        {/* Undo / Redo */}
        <div
          className={`hidden xs:flex items-center p-0.5 rounded-xl border ${
            isLight
      ...
      {/* CENTER SECTION: Viewport Mode Switcher */}
      <div
        className={`flex items-center p-1 rounded-xl border shadow-inner shrink-0 ${
          isLight
      ...
      >
        <button
          ...
          className={`flex items-center space-x-1 px-1.5 sm:px-2 py-1 rounded-lg text-xs font-medium transition-all ${
            viewMode === "desktop"
        ...
        <button
          ...
          className={`hidden sm:flex items-center space-x-1 px-1.5 sm:px-2 py-1 rounded-lg text-xs font-medium transition-all ${
            viewMode === "tablet"
              ? "bg-indigo-600 text-white shadow-sm font-bold"
              : isLight
                ? "text-slate-600 hover:text-slate-900 hover:bg-slate-200"
        ...
        <div
          className={`hidden md:block h-4 w-px mx-1 ${isLight ? "bg-slate-300" : "bg-slate-800"}`}
        />

        <button
          onClick={() =>
            setViewMode(viewMode === "preview" ? "desktop" : "preview")
          }
          className={`hidden md:flex items-center space-x-1 px-2 py-1 rounded-lg text-xs font-medium transition-all ${
            viewMode === "preview"
      ...
      <div className="flex items-center space-x-1.5 shrink-0">
        ...
        <button
          ...
          title={
            boxInspectorEnabled
              ? "Box Model Inspector Active"
              : "Toggle Visual Box Model & Spacing Inspector"
          }
        >
        ...
        <button
          onClick={() => {
            resetCanvas();
            addToast("Canvas reset to default root layout.", "info");
          }}
        ...
        <button
          ...
          title={
            isRightSidebarOpen
              ? "Collapse Style Inspector"
              : "Expand Style Inspector"
          }
        >
          {isRightSidebarOpen ? (
